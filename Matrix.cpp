...
void Matrix_print(const Matrix* mat, std::ostream& os)
{
    ...
    for (int i = 0; i < ((mat->height) * (mat->width)); i = (i + (mat->width)))
    {
        for (int j = i; j < (i + (mat->width)); j++)
        {
            os << mat->data[j] << " ";
        }
        os << std::endl;
...
{
    int point_diff = static_cast<int>(ptr - (mat->data));
    int row = (int)((point_diff) / (mat->width));
    return row;
}
    ...
    for (int row = 0; row < (mat->height); row++)
    {
        for (int col = 0; col < (mat->width); col++)
        {
            if ((row == 0) || (row == ((mat->height) - 1)) ||
                (col == 0) || (col== ((mat->width) - 1)))
            {
                mat->data[((mat->width) * row) + col] = value;
            }
        }
    }
}
                                      ...
                                      int column_start, int column_end)
{
    ...
    assert(column_start >= 0 && column_end <= (mat->width));
